# 소셜 미디어 API 데이터 구조 통합 문서

Samsung 브랜드 모니터링을 위한 YouTube, Instagram, TikTok API 데이터 구조 분석

---

# 목차

1. [YouTube API 데이터 구조](#1-youtube-api-데이터-구조)
2. [Instagram API 데이터 구조](#2-instagram-api-데이터-구조)
3. [TikTok API 데이터 구조](#3-tiktok-api-데이터-구조)
4. [플랫폼별 비교](#4-플랫폼별-비교)

---

# 1. YouTube API 데이터 구조
...
| 파라미터 | 필수 | 타입 | 기본값 | 설명 |
|----------|------|------|--------|------|
| `q` | 필수 | string | - | 검색 키워드 (예: "samsung tv") |
| `maxResults` | 선택 | integer | 5 | 반환할 최대 결과 수 (1-50) |
| `regionCode` | 선택 | string | US | 국가 코드 (ISO 3166-1 alpha-2) |
| `pageToken` | 선택 | string | - | 페이지네이션 토큰 (다음/이전 페이지) |
| `publishedAfter` | 선택 | datetime | - | 이 날짜 이후 업로드된 동영상 (RFC 3339 형식) |
| `publishedBefore` | 선택 | datetime | - | 이 날짜 이전 업로드된 동영상 (RFC 3339 형식) |
| `order` | 선택 | string | relevance | 정렬 기준 (relevance, date, viewCount, rating) |
| `type` | 선택 | string | video,channel,playlist | 결과 유형 (video, channel, playlist) |

### 페이지네이션

YouTube는 **pageToken** 방식을 사용합니다:
- **첫 요청**: `pageToken` 없이 요청
- **다음 페이지**: 응답의 `nextPageToken` 사용
- **이전 페이지**: 응답의 `prevPageToken` 사용
- **제한 없음**: `nextPageToken`이 있는 한 계속 수집 가능

```python
# YouTube 페이지네이션 예시 (기간 필터링 포함)
def collect_youtube_videos(keyword, start_date=None):
    page_token = None
    all_videos = []
    ...
    while True:
        ...
        # 기간 필터 (선택사항)
        if start_date:
            params["publishedAfter"] = start_date  # "2025-01-01T00:00:00Z"
...
YouTube Data API v3의 **실제 제한사항**:

1. **할당량 제한**
   - 프로젝트당 일일 **10,000 units** 기본 제공
   - `search.list`: 1회 호출당 **100 units** 소모
   - 하루 최대 약 **100회** 검색 가능 (다른 API 호출 없이)

2. **결과 수 제한**
   - 한 번에 최대 **50개** 결과만 요청 가능 (`maxResults`)
   - 페이지네이션은 **제한 없음** (관련성 낮은 결과는 자연스럽게 종료)

3. **검색 결과 순서**
   - 관련성 기반 정렬 시 페이지가 깊어질수록 관련성 낮은 결과 반환
   - 날짜순 정렬(`order=date`) 사용 시 최신 순서 보장

4. **다계정 우회**
   - 여러 API 키 로테이션으로 할당량 제한 우회 가능 (비공식)
   - 각 프로젝트마다 별도의 10,000 units 제공

---

# 2. Instagram API 데이터 구조
...
| 플랫폼 | 방식 | 파라미터 | 특징 |
|--------|------|----------|------|
| **YouTube** | Token 기반 | `pageToken` | 응답의 `nextPageToken`을 다음 요청에 사용 |
| **Instagram** | 해시태그/사용자 기반 | N/A | 해시태그나 사용자 ID로 검색 (페이지네이션 정보 미제공) |
| **TikTok** | Cursor + Session ID | `cursor`, `search_id` | cursor는 위치, search_id는 세션 추적용 |

## 데이터 필드 수 비교

| 플랫폼 | 총 필드 수 | 필수 필드 | 권장 필드 | 불필요 필드 |
|--------|------------|-----------|-----------|-------------|
| **YouTube** | N/A | - | - | - |
| **Instagram** | 45 | 22 | 16 | 7 |
| **TikTok** | 164 | 38 | 71 | 55 |

## 핵심 인게이지먼트 지표 비교

| 플랫폼 | 조회수 | 좋아요 | 댓글 | 공유 | 저장 | 재생시간 |
|--------|--------|--------|------|------|------|----------|
| **YouTube** | `viewCount` | `likeCount` | `commentCount` | - | - | `duration` |
| **Instagram** | `view_count` | `like_count` | `comment_count` | - | - | `video_duration` |
| **TikTok** | `playCount` | `diggCount` | `commentCount` | `shareCount` | `collectCount` | `video.duration` |

## 작성자 정보 비교

| 플랫폼 | 사용자 ID | 사용자명 | 닉네임 | 팔로워 수 | 인증 여부 |
|--------|-----------|----------|--------|-----------|-----------|
| **YouTube** | `channelId` | `channelTitle` | - | `subscriberCount` | `verified` |
| **Instagram** | `user.pk` | `user.username` | `user.full_name` | - | `user.is_verified` |
| **TikTok** | `author.id` | `author.uniqueId` | `author.nickname` | `authorStats.followerCount` | `author.verified` |

## 콘텐츠 타입 비교

| 플랫폼 | 이미지 | 비디오 | 캐러셀/멀티 | 쇼츠/릴스 |
|--------|--------|--------|-------------|-----------|
| **YouTube** | ❌ | ✅ | ❌ | ✅ (Shorts) |
...
| 플랫폼 | 검색 파라미터 | 해시태그 필드 | 키워드 분석 |
|--------|---------------|---------------|-------------|
| **YouTube** | `q` (키워드) | `tags[]` | 제목, 설명에서 추출 |
| **Instagram** | 해시태그 검색 | `caption_text` | 캡션에서 `#` 파싱 필요 |
| **TikTok** | `keyword` | `challenges[]`, `textExtra[]` | 구조화된 해시태그 제공 |

## 광고 식별 비교

| 플랫폼 | 광고 필드 | 유료 파트너십 | FTC 공개 |
|--------|-----------|---------------|----------|
| **YouTube** | `isSponsored` | - | - |
| **Instagram** | - | `is_paid_partnership` | - |
| **TikTok** | `isAd` | - | `author.ftc` |

## URL 만료 정책 비교

| 플랫폼 | 미디어 URL 만료 | 권장 처리 |
|--------|-----------------|-----------|
| **YouTube** | 6시간 | 즉시 다운로드 또는 재생성 |
| **Instagram** | 24시간 (추정) | 즉시 다운로드 권장 |
| **TikTok** | 1-2시간 (추정) | 즉시 다운로드 필수 |

## Samsung 브랜드 모니터링 추천 전략

### 플랫폼별 강점

| 플랫폼 | 강점 | 권장 사용 사례 |
|--------|------|----------------|
| **YouTube** | 긴 형식 리뷰, 언박싱 영상 | 제품 상세 리뷰, 튜토리얼 |
| **Instagram** | 제품 이미지, 인플루언서 마케팅 | 비주얼 트렌드, 스폰서십 분석 |
| **TikTok** | 바이럴 쇼트폼, 젊은 층 | 트렌드 파악, 빠른 반응 측정 |
...
# 통합 키워드 전략
samsung_keywords = {
    "youtube": ["samsung tv review", "samsung galaxy unboxing"],
    "instagram": ["#samsung", "#samsunggalaxy", "#galaxys25"],
    "tiktok": ["samsung", "samsung phone", "galaxy s25"]
}

# 수집 주기
collection_schedule = {
    "youtube": "6시간마다",      # 긴 콘텐츠, 덜 빈번
    "instagram": "3시간마다",    # 중간 속도
    "tiktok": "1시간마다"        # 빠른 바이럴, 자주 수집
}
```
...
1. **YouTube**: 제품 리뷰 및 심층 분석 콘텐츠 수집
2. **Instagram**: 비주얼 마케팅 및 인플루언서 파트너십 모니터링
3. **TikTok**: 실시간 트렌드 및 바이럴 콘텐츠 추적

각 플랫폼의 데이터를 통합하여 Samsung 브랜드에 대한 360도 소셜 미디어 모니터링 시스템 구축을 권장합니다.
