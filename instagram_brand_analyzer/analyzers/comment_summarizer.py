...
# Import OpenAI API key from parent config
try:
    ...
    secrets_path = os.path.join(parent_dir, 'config', 'secrets.py')
...
class CommentSummarizer:
    ...
    def summarize_comments_for_video(self, comments: List[Dict], max_comments=100) -> Dict:
        ...
        for comment in top_comments:
            # YouTube: comment_text_original, Instagram: comment_text
            text = comment.get('comment_text_original', comment.get('comment_text', ''))
